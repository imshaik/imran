...
try {
    ...
    node('Docker') 
    {
        ...
        */
		...
		def dockerimage = docker.build("${nexushost}/${dockerreponame}:${buildlabel}").push()
	...
	stage('Remove local images & containers') 
	{
		...
		echo "Removing the image ${nexushost}/${dockerreponame}:${buildlabel}"
		sh("docker rmi -f ${nexushost}/${dockerreponame}:${buildlabel}")*/
